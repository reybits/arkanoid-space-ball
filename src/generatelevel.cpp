// GenerateLevel.cpp: implementation of the CGenerateLevel class.
//
//////////////////////////////////////////////////////////////////////
...
CGenerateLevel::~CGenerateLevel() {
	g_Resource.FreeMem((unsigned char*)m_pbyLevelPatterns);
}
...
void CGenerateLevel::Generate(int nLevel, bool bUseCustom) {
	...
	if(bUseCustom == true) {
		...
		/*for(int x = 0; x < BRICKS_WIDTH; x++) {
			Uint8	byType	= 1 + g_Rnd.Get(BOX_END - 1);
		...
		if(nLevel < 10) {
			...
			//do {
				...
				/*if(_CalculateBricksCount() > 100) {// && nLevel < 15) {
					int	nDx	= (1 + g_Rnd.Get(6));
					int	nDy	= (1 + g_Rnd.Get(5));
	...
	for(int y = 0; y < BRICKS_HEIGHT; y++) {
		for(int x = 0; x < BRICKS_WIDTH; x++) {
			...
			if(byType > BOX_NONE && byType < BOX_END) {
				...
				brick.bDir				= byType == BOX_MOV_0 ? (g_Rnd.Get(2) == 0 ? false : true) : (byType == BOX_MOV_1 ? true : false);
				brick.nDirChangeCount	= 0;
				brick.nCountToShoot		= 5 + nCountToShoot;
				g_Arkanoid.m_vecLevelBricks.push_back(brick);
				if(byType >= BOX_SHOOT_0 && byType < BOX_BOMB_0)	nCountToShoot	+= 5;
			}
		}
		//printf("\n");
	}
...
void CGenerateLevel::_FillLevel(int nLevel, int nPos) {
	int	nPattern;
	do {
		nPattern	= g_Rnd.Get(m_nPatterns);
	...
	for(int x = nPos * 3; x < nPos * 3 + 3; x++) {
		for(int y = 0; y < BRICKS_HEIGHT; y++) {
			...
			//byType	= min(byType, g_Rnd.Get(BOX_END));
...
void CGenerateLevel::_RollLevel() {
	if(g_Rnd.Get(2) == 0) {
		int	nRollCount	= g_Rnd.Get(BRICKS_WIDTH - 1);
		for(int i = 0; i < nRollCount; i++) {
			_RollHorizontal();
		}
	}
	else {
		int	nRollCount	= g_Rnd.Get(BRICKS_HEIGHT - 1);
		for(int i = 0; i < nRollCount; i++) {
			_RollVertical();
		}
	}
...
void CGenerateLevel::LoadPatterns() {
	...
	m_pbyLevelPatterns	= (Uint8*)g_Resource.GetDataAllocMem("patterns.bin", nDataLen);
